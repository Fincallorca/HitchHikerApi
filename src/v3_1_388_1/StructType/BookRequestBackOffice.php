<?php

namespace Fincallorca\HitchHikerApi\Wsdl\v3_1_388_1\StructType;

use \WsdlToPhp\PackageBase\AbstractStructBase;

/**
 * This class stands for BookRequestBackOffice StructType
 * Meta informations extracted from the WSDL
 * - nillable: true
 * - type: tns:BookRequestBackOffice
 * @subpackage Structs
 */
class BookRequestBackOffice extends AbstractStructBase
{
    /**
     * The CustomerReferenceNumber
     * Meta informations extracted from the WSDL
     * - minOccurs: 0
     * - nillable: true
     * @var string
     */
    public $CustomerReferenceNumber;
    /**
     * The InformationText
     * Meta informations extracted from the WSDL
     * - minOccurs: 0
     * - nillable: true
     * @var string
     */
    public $InformationText;
    /**
     * Constructor method for BookRequestBackOffice
     * @uses BookRequestBackOffice::setCustomerReferenceNumber()
     * @uses BookRequestBackOffice::setInformationText()
     * @param string $customerReferenceNumber
     * @param string $informationText
     */
    public function __construct($customerReferenceNumber = null, $informationText = null)
    {
        $this
            ->setCustomerReferenceNumber($customerReferenceNumber)
            ->setInformationText($informationText);
    }
    /**
     * Get CustomerReferenceNumber value
     * An additional test has been added (isset) before returning the property value as
     * this property may have been unset before, due to the fact that this property is
     * removable from the request (nillable=true+minOccurs=0)
     * @return string|null
     */
    public function getCustomerReferenceNumber()
    {
        return isset($this->CustomerReferenceNumber) ? $this->CustomerReferenceNumber : null;
    }
    /**
     * Set CustomerReferenceNumber value
     * This property is removable from request (nillable=true+minOccurs=0), therefore
     * if the value assigned to this property is null, it is removed from this object
     * @param string $customerReferenceNumber
     * @return \Fincallorca\HitchHikerApi\Wsdl\v3_1_388_1\StructType\BookRequestBackOffice
     */
    public function setCustomerReferenceNumber($customerReferenceNumber = null)
    {
        // validation for constraint: string
        if (!is_null($customerReferenceNumber) && !is_string($customerReferenceNumber)) {
            throw new \InvalidArgumentException(sprintf('Invalid value, please provide a string, "%s" given', gettype($customerReferenceNumber)), __LINE__);
        }
        if (is_null($customerReferenceNumber) || (is_array($customerReferenceNumber) && empty($customerReferenceNumber))) {
            unset($this->CustomerReferenceNumber);
        } else {
            $this->CustomerReferenceNumber = $customerReferenceNumber;
        }
        return $this;
    }
    /**
     * Get InformationText value
     * An additional test has been added (isset) before returning the property value as
     * this property may have been unset before, due to the fact that this property is
     * removable from the request (nillable=true+minOccurs=0)
     * @return string|null
     */
    public function getInformationText()
    {
        return isset($this->InformationText) ? $this->InformationText : null;
    }
    /**
     * Set InformationText value
     * This property is removable from request (nillable=true+minOccurs=0), therefore
     * if the value assigned to this property is null, it is removed from this object
     * @param string $informationText
     * @return \Fincallorca\HitchHikerApi\Wsdl\v3_1_388_1\StructType\BookRequestBackOffice
     */
    public function setInformationText($informationText = null)
    {
        // validation for constraint: string
        if (!is_null($informationText) && !is_string($informationText)) {
            throw new \InvalidArgumentException(sprintf('Invalid value, please provide a string, "%s" given', gettype($informationText)), __LINE__);
        }
        if (is_null($informationText) || (is_array($informationText) && empty($informationText))) {
            unset($this->InformationText);
        } else {
            $this->InformationText = $informationText;
        }
        return $this;
    }
    /**
     * Method called when an object has been exported with var_export() functions
     * It allows to return an object instantiated with the values
     * @see AbstractStructBase::__set_state()
     * @uses AbstractStructBase::__set_state()
     * @param array $array the exported values
     * @return \Fincallorca\HitchHikerApi\Wsdl\v3_1_388_1\StructType\BookRequestBackOffice
     */
    public static function __set_state(array $array)
    {
        return parent::__set_state($array);
    }
    /**
     * Method returning the class name
     * @return string __CLASS__
     */
    public function __toString()
    {
        return __CLASS__;
    }
}
